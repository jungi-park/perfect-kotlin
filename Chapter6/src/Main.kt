...
//    val WeekDay.nextDay  get() = weekDays[(ordinal+1)/weekDays.size]
    ...
    * */
    ...
    /* 6.3.1 인라인 클래스 정의하기
    인라인 클래스를 정의하려면 value class를 클래스 이름 앞에 붙여야하낟.
    ...
    인라인 클래스의 주생성자에는 불변 프로퍼티를 하나만 선언해야 한다. 런타임에 클래스 인스턴스는 별도의 래퍼 객체를 생성하지 않고
    이 프로퍼티의 값으로 표현된다. 그래서 인라인 클래스라는 이름이 붙었다. 런타임에는 호출 지점을 함수 본문으로 인라인하는 인라인 함수처럼
    인라은 클래스 객체를 사용하는 위치 대신 인라인 클래스에 들었있는 값이 들어간다.
